...
/* sh_quote_procs.c */
void			sh_handle_unclosed_quotes(char **line, int tmp_fd,
					t_quote_state state);
...
/* sh_tree_build.c */
t_sh_node		*sh_cmd_token(t_sh_token **tokens, t_sh_env **env,
					t_sh_token *tmp);
void			sh_brace_tree(t_sh_token **tk, t_sh_node **tree,
					t_sh_env **env);
void			sh_connect_ops(t_sh_token **tk, t_sh_node **tree,
					t_sh_env **env);
t_sh_node		*sh_build_tree(t_sh_token *tokens, t_sh_env **env);

/* sh_var_expand.c */
int				sh_get_env_var_length(char *str);
void			sh_process_quoted_section(char **s, int *rem,
					t_quote_state *state);
void			sh_insert_env_vars(t_sh_env *env, char *s, char ***res_parts,
					t_quote_state state);
void			sh_replace_env_vars(t_sh_env *env, char **line,
					t_quote_state state);

/* sh_wildcard_matcher.c */
int				sh_match_wildcard(const char *filename, const char *pattern);
char			**sh_get_matching_files(char *pattern);
void			sh_expd_wildcard_token(t_sh_token **token_list,
					t_sh_token **current);
