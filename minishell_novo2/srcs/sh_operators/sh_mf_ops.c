...
/*   Updated: 2025/05/12 15:08:06 by jnovack          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
error_t     sh_output_load(t_sh_cmd *cmd)
{
    ...
    while (tmp && cmd->output_fd != FILE_ERROR && op == ERR_NONE)
    {
        ...
        if (tmp->mode == REDIR_OUTPUT)
            op = sh_heredoc_write(cmd, ft_strdup(tmp->target), FILE_MODE_WRITE_CREATE);
        else if (tmp->mode == REDIR_APPEND)
            op = sh_heredoc_write(cmd, ft_strdup(tmp->target), FILE_MODE_WRITE_APPEND);
        if (cmd->output_fd != FILE_ERROR)
            tmp = tmp->next;
    }
