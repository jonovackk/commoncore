...
/*   Updated: 2025/05/12 15:07:39 by jnovack          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    sh_run_cmd(t_sh_cmd *cmd, int *node_fd, t_sh_exec *exec_ctx)
{
    ...
    if (child_pid == 0)
    {
        ...
        sh_process_redirections(cmd, node_fd);
    ...
    sh_cmd_cleanup(cmd);
    sh_pid_push(&(exec_ctx->running_procs), sh_init_pid(child_pid)); 
}
...
void    sh_handle_command(t_sh_node *tree, int *node_fd, t_sh_exec *exec_ctx, exec_t mode)
{
    ...
    err_str = NULL;
    command = tree->cmd;
    if (sh_init_command(command, exec_ctx) || sh_check_command(command, exec_ctx))
        return;
